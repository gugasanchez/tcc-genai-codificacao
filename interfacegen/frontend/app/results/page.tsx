...
export default function ResultsPage() {
  ...
  if (!Number.isFinite(sessionId)) {
    return (
      <div className="p-6 text-red-700">Parâmetro sessionId inválido.</div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-white">
        ...
        <Link
          href={
            runId
              ? `/results-compare?runId=${runId}`
              : `/results?sessionId=${data.id}&view=avaliar`
          }
      ...
      <div className="mx-auto max-w-3xl px-6 py-8">
        <header className="mb-6 flex items-center justify-between">
          <h1 className="text-2xl font-semibold">
            Avaliação da sessão #{data!.id}
          </h1>
          <Link
            href={`/results?sessionId=${data!.id}&view=preview${
              runId ? `&runId=${runId}` : ""
            }`}
        ...
        <section className="rounded-lg border bg-white p-5 shadow-sm">
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-zinc-700">
            <li>
              <strong>Acessibilidade (WCAG 2.1)</strong>: o sistema roda
              auditoria automática (axe/Lighthouse) e exibe um score 0–100.
              Considere contraste, labels, alt em imagens, navegação por
              teclado.
            </li>
            <li>
          ...
          <div className="mt-4 rounded-md bg-zinc-50 p-3 text-xs text-zinc-600">
            Dica: você pode revisar o preview em tela cheia a qualquer momento
            pelo link no topo desta página.
          </div>
        </section>
        ...
        <section className="mt-6 rounded-lg border bg-white p-5 shadow-sm">
          ...
          <p className="mb-2 mt-1 text-xs text-zinc-600">
            Preencha para a sessão #{data!.id}
          </p>
